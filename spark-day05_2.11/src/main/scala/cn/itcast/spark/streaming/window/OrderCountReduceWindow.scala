...
/**
  ...
  */
object OrderCountReduceWindow {

	// 检查点目录
	val CHECKPOINT_PATH: String = "datas/streaming/state-order-223332332"
	...
	def processStreaming(ssc: StreamingContext): Unit = {
		...
		// TODO：将窗口Window与聚合函数合在一起使用
		/*
		  def reduceByKeyAndWindow(
			  reduceFunc: (V, V) => V,   // 设置如何聚合统计
			  windowDuration: Duration, // 窗口大小的设置
			  slideDuration: Duration // 活动大小的设置
			): DStream[(K, V)]
		 */
		val provinceCountDStream = orderDStream.reduceByKeyAndWindow(
			(v1: Int, v2: Int) => v1 + v2 ,
